-- Assignments using Banking dataset from session 5:
1) Create a new BRANCH table having BRANCH_CODE, BRANCH_NAME and HEAD_OFFICE_FLAG fields. Load 4 records for 4 different cities. Make one of them as head office by making HEAD_OFFICE_FLAG = "Y".

2) Without droping the tables, Add new column "BRANCH" to the tables CUSTOMERS and EMPLOYEES. Make sure by default this column is populated with head office branch code.

3) Modify the branch value in CUSTOMERS and EMPLOYEES table such that you have customers and employees from each branch.
...
-- NOW using the Sales Order dataset.


-- Create new sales_order database
create database sales_order;




select to_date('10-02-2009', 'dd-mm-yyyy');

SELECT CONVERT(DATETIME, '10-02-2009', 102);

select cast('10-02-2009' as date); -- SQL Server
 

select "Lighthall", 'Lighthall';
