...
a) Find the total users on the platform.
b) Find the total no of frnds for each user
c) pop perc = (b / a) * 100 = %
--
with tot_users as 
	 ...
	 no_of_frnds as 
	 		(select users, count(1):: decimal as tot_frnds
			 from tot_users
			 group by users),
	tot_no_of_users as
			(select count(distinct users):: decimal as total_user_count from tot_users)
select users, round((tot_frnds/total_user_count)*100,2) as pop_perc 
from no_of_frnds
cross join tot_no_of_users  
order by users;
--
-- ASSIGNMENT
-- User Email Labels
